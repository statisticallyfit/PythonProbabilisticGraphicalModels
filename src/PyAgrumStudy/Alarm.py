...
# Creating the brief random variables
Burglary_brief = RandomVariable(var = )
...
indepSynonymTable(model = alarmModel, query= MaryCalls)
# %% codecell
print(alarmModel.local_independencies(JohnCalls.var))

indepSynonymTable(model = alarmModel, query= JohnCalls)
# %% codecell
...
print(indepSynonymTable(model = alarmModel, query= Burglary))
print(indepSynonymTable(model = alarmModel, query= Earthquake))
...
# $$
# \color{Green}{ \text{Alarm (unobserved): }\;\;\;\;\;\;\;\;\; \text{Burglary} \longrightarrow \text{Alarm} \longleftarrow \text{Earthquake}}
# $$

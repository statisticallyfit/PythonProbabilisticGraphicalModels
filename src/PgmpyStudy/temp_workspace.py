...
roots: List[Name] = model.get_roots(); roots
leaves: List[Name] = model.get_leaves(); leaves
...
interimNodes: List[Name] = list(filter(lambda node: not (node in roots) and not (node in leaves), model.nodes())); interimNodes


# Returns dict {varfromvarlist : [children]}
def nodeChildPairs(model: BayesianModel, vars: List[Name]) -> Dict[Name, List[Name]]:
    return [{node : list(model.successors(n = node))} for node in vars]

rootPairs: Dict[Name, List[Name]] = nodeChildPairs(model, roots); rootPairs
